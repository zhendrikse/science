...
vec4 raytrace(vec3 rayDir, vec3 rayPos) {
    ...
    for (int i = 0; i < MAX_ITERATIONS; i++) {
        ...
        if (dist > innerDiskRadius && dist < outerDiskRadius && rayPos.y * steppedRayPos.y < 0.0) {
            ...
            diskDensity *= densityVariation * invSqrtDist * invSqrtDist + 0.5 * fbm(
                rotate(rayPos, vec3(0.0, -uTime * (1.0 / abs(diskDist)) * 2.0, 0.0)),
                3, 5.0, 0.1, 0.5
            );
            ...
            vec3 innerColor = vec3(2.0, 1.6, 1.1);
            vec3 midColor = vec3(1.2, 0.6, 0.15);
            vec3 outerColor = vec3(0.8, 0.15, 0.05);

            vec3 diskColor = mix(mix(outerColor, midColor, temp), innerColor, temp * temp * temp);

            float photonRadius = 1.5;
            float photonThickness = 0.03;
...
void main() {
    ...
    vec3 rayDir = rotate(normalize(vec3(uv, 1.0)), uRotation);
