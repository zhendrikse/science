...
class Comet:
    def __init__(self, start_position, radius, colour, sour=None):
        self._sphere = sphere(pos=start_position, radius=radius, color=colour, visible=True, make_trail=True)
        self._sour = sour
        self._start_sour = [sour[0], sour[1], sour[2], sour[3]] if sour is not None else None
    ...
    def reset(self):
        ...
        self._sour = [self._start_sour[0], self._start_sour[1], self._start_sour[2], self._start_sour[3]] if self._sour is not None else None
...
comet = Comet(start_position=position, radius=0.8, colour=color.cyan, sour=[start_r, 0, -25.2, 0.49])
flat_motion_comet = Comet(vec(position.x, schwarzschild.grid_y_offset(), position.z), 0.8, color.red)
