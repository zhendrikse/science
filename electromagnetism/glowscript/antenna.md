```python
Web VPython 3.2
from vpython import vector, canvas, color, cylinder, arrow, rate, sin, cos, checkbox, pi, norm, mag, cross, slider
...
class ElectromagneticWave:
    ...
    def update(self):
        for index in range(len(self._electric_field)):
            ...
            field_arrow.axis = vector(0, decrease * self._E0 * cos(omega * t - 2 * pi * mag(field_arrow.pos - slit1) / lambda_), 0)
