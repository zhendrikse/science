...
title="""&#x2022; Original <a href="https://www.glowscript.org/#/user/Luinthoron/folder/English/program/embedding-diagram">code.py</a> by M. Ryston
&#x2022; Department of Physics Education, Faculty of Mathematics and Physics, Charles University, V Holešovičkách 2, 180 00, Prague, Czech Republic
...
display = canvas(title="Schwarzschild Space Time", width=600, height=450, background=color.gray(0.075), forward=vec(0, -.45, -.85), range=62)
...
class Sun:
    def __init__(self, position, radius=0.8):
        self._sun = sphere(pos=position , radius=radius, texture="https://www.hendrikse.name/science/astrophysics/images/textures/sun.jpg", visible=True)
...
class Comet:
    ...
    def update_by(self, M, dt):
        ...
        buff[2] = self._sour[2] + 0.5 * dt * (M / self._sour[0] / (self._sour[0] - 2.0 * M) * self._sour[2] ** 2 + (self._sour[0] - 2.0 * M) * self._sour[3] ** 2)
        ...
        self._sphere.pos = vec(self._sour[0] * cos(self._sour[1]), sqrt(8 * M * self._sour[0] - 16 * M ** 2), self._sour[0] * sin(self._sour[1]))
...
class SchwarzschildSpaceTime:
    ...
    def draw_grid(self):
        ...
        for i in range(20):
            self._grid += [curve(
                pos=[vec(-r_max + i * 2 * r_max / 19, self._grid_y_offset, -r_max), vec(-r_max + i * 2 * r_max / 19, self._grid_y_offset, r_max)],
                color=color.white, radius=0.2)]
            self._grid += [curve(
                pos=[vec(-r_max, self._grid_y_offset, -r_max + i * 2 * r_max / 19), vec(r_max, self._grid_y_offset, -r_max + i * 2 * r_max / 19)],
                color=color.white, radius=0.2)]
...
start_r = r_max - 0.1
comet = Comet(start_position=vec(start_r, schwarzschild.z_as_function_of(start_r), 0), particle_radius=0.8, colour=color.cyan, sour = [start_r, 0, -25.2, 0.49])
