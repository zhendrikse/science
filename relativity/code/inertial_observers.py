from vpython import vector, canvas, box, arrow, text, sphere, cylinder, rate, color, sin, cos, local_light

title="""&#x2022; Original <a href="https://www.mso.anu.edu.au/pfrancis/simulations/barges_light.barges_light">XRB.py</a> by <a href="http://www.mso.anu.edu.au/pfrancis/simulations/">Paul Francis</a>
...
scene = canvas(title=title, range=3, width=650, height=400, forward=vector(0.2, -0.3, -1.0), background=color.gray(0.075))
water = box(pos=vector(0, -0.1, 0.0), length=10, width=3, height=0.1, color=vector(0.1, 0.2, 1.0))
...
class Barge:
    ...
    def switch_light_on(self):
        self._light_light.visible = True
        self._light.color = color.white
...
while 1:
    ...
    if t == 400:
        barge1.switch_light_on()
        barge2.switch_light_on()
