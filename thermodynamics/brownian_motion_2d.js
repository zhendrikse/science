...
const brownianMotion2d = document.getElementById("brownianMotion2d");
const temperatureSlider = document.getElementById("temperatureSlider2d");
const centerX = brownianMotion2d.clientWidth  * .5;
const centerY = brownianMotion2d.clientHeight * .5;
const scene = new Scene();

const camera = new OrthographicCamera(-centerX, centerX, centerY, -centerY, 0.1, 1000);
camera.position.z = 10;

const renderer2d = new WebGLRenderer({ antialias: true, canvas: brownianMotion2d, alpha: true });
renderer2d.setClearColor(0x000000, 0); // alpha 0
renderer2d.setSize(brownianMotion2d.clientWidth, brownianMotion2d.clientHeight);
renderer2d.setAnimationLoop( animate );

const boxSize = brownianMotion2d.clientWidth * .9; // keep margin
const binCount = 30;
const maxSpeed = 5
const gas = new Gas2D({
...
const speedPlot = new uPlot({
        ...
        width: brownianMotion2d.clientWidth,
        height: brownianMotion2d.clientHeight * .5,
        bg: "transparent",
        scales: {
            x: { time: false },
