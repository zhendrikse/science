...
import { SkyDome, Earth, Planet, Moon, Sun, PlanetMoonSystem, Saturn, Uranus, SCALE_MOON } from '../js/astro-extensions.js';
...
const planetCamera = new PerspectiveCamera(50, 1, .1, 7500);
...
const skyDome = new SkyDome({starDensity: 1, skyRadius: 7500, glowStarCount: 500});
...
export class LunarEclipseIndicator {
    constructor(earth, moon, plane) {
        ...
        this._threshold = earth.scaledRadius * 0.05; // ± 5% van straal
        this._active = false;
...
class ControlsGui {
    constructor() {
        ...
        controls.add(settings, 'equatorialPlane')
            ...
            .onChange(value => Bodies.earth.equatorialPlane.visible = value);
        controls.add(settings, 'earthClouds')
            .name("Earth clouds")
            .onChange(value => Bodies.earth.clouds.visible = value);
...
Bodies.earth.clouds.visible = false;
Bodies.sun.position.set(0, 0, 0);
solarSystemGroup.add(Bodies.sun);
...
Bodies.earth.enableTrail({maxPoints: 400, color: 0x00bbff, trailStep: 125});
...
Bodies.mercury.enableTrail({maxPoints: 80, color: 0xaaff00, trailStep: 25});
...
Bodies.venus.enableTrail({maxPoints: 120, color: 0xff0000, trailStep: 100});
...
Bodies.mars.enableTrail({maxPoints: 550, color: 0xffaa00, trailStep: 250});
...
Bodies.jupiter.enableTrail({maxPoints: 1600, color: 0x8b4513, trailStep: 500});
...
Bodies.saturn.enableTrail({maxPoints: 1600, color: 0xffff00, trailStep: 500});
...
Bodies.uranus.enableTrail({maxPoints: 1600, color: 0xaaaaaa, trailStep: 500});
...
Bodies.neptune.enableTrail({maxPoints: 1600, color: 0x0000ff, trailStep: 500});
...
const eclipticPlane = new Mesh(
    new RingGeometry(Bodies.sun.scaledRadius * 1.1, 100, 128),
...
document.querySelectorAll("#planetControls button[data-body]")
    .forEach(button => {
        button.addEventListener("click", () => {
            const key = button.dataset.body; // "jupiter", "saturn", etc.
...
let tPhys = 0;
const eclipseIndicator = new LunarEclipseIndicator(Bodies.earth, Moons.luna, Bodies.earth.equatorialPlane);
